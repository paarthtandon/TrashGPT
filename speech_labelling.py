...
def reload(fileName):
    return pd.read_csv('speech_diarization/' + fileName, sep=' ', names=["type", "file", "channel", "start", "duration","NA1", "NA2", "speaker", "NA3", "NA4"])
...
def fileNotEditted(fileName):
    file = pd.read_csv('speech_diarization/' + fileName, sep=' ', names=["type", "file", "channel", "start", "duration","NA1", "NA2", "speaker", "NA3", "NA4"])
...
def analyzeInput(pdFile, audiofile, speaker):
    ...
    if userInput== 'label':
        ...
        if confirm == 'yes':
            ...
            pdFile.to_csv('speech_diarization/' + filename, sep=' ', index=False, header=False)
    ...
    if userInput== 'audio':
        ...
        try:
            ...
            if len(duration) < 3:
                print('1 audio for this speaker with a duration greater than 10 seconds')
...
pathtodata = pathlib.Path('C:/Users/Ed Zaharof/Documents/GitHub/TrashGPT/speech_diarization')
filesinlib = os.listdir(pathtodata)
for filename in filesinlib:
    file = fileNotEditted(filename)
    if file is not None:
        print(f'Working on: {filename}')
        audio = AudioSegment.from_file('data/' + filename[:-4] + 'm4a')
