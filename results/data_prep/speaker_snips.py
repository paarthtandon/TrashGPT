...
def fileNotEditted(fileName):
    file = pd.read_csv(read_dir + fileName, sep=' ', names=["type", "file", "channel", "start", "duration","NA1", "NA2", "speaker", "NA3", "NA4"])
...
def analyzeInput(pdFile, audiofile, speaker):
    ...
    if userInput== 'label':
        ...
        if confirm == 'yes':
            ...
            pdFile.to_csv(write_dir + filename, sep=' ', index=False, header=False)
...
pathtodata = pathlib.Path('../annotated_w_names/')
